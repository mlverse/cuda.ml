...
- `CUML_PREFIX`: existing RAPIDS cuML 24.0 or newer prefix containing
  `include/cuml` and `lib/libcuml.so`.
...
If you already have RAPIDS cuML 24.0 or newer from pip, conda, or a source
build, set `CUML_PREFIX` to a prefix containing `include/cuml` and
`lib/libcuml.so` before installing {cuda.ml}. In this case the automatic
