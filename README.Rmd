...
no-op. Managed builds use the package's portable GPU architecture list by
default, which permits GPU-free build hosts and reuse across supported GPUs. To
compile only for the distinct compute capabilities reported by `nvidia-smi`,
run:
...
supports only the detected GPU architectures. You can instead supply an
