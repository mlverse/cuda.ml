...
The simplest file workflow writes the model state to an open binary connection.
The file contains explicit model state rather than a live native pointer.
...
In a new R process, prepare the required backend, open the file for binary
reading, and restore the model.
...
With its default `connection = NULL`, `cuda_ml_serialize()` returns the complete
state as a raw vector. This is useful for object stores and other systems that
...
cuda.ml records the package and backend details needed to check a state before
restoring it. A different cuda.ml package version does not by itself prevent
restoration. Native backend requirements depend on the model family.
...
An unsupported model state or an incompatible required backend version produces
an error before the model payload is restored.
...
the original fit. cuda.ml does not provide warm-start, incremental-training, or
fine-tuning operations for either live or restored fits. To refit a model, call
its fitting function again with the training data.
