...
Rcpp::List pca_fit_transform(Rcpp::NumericMatrix const& x, double const tol,
  ...
  auto const n_rows = m.numRows;
  auto const n_cols = m.numCols;
  ...
  result["components"] = Rcpp::transpose(
    Rcpp::NumericMatrix(n_components, n_rows, h_components.begin()));
  result["explained_variance"] =
    Rcpp::NumericMatrix(n_components, 1, h_explained_var.begin());
  result["explained_variance_ratio"] =
    Rcpp::NumericMatrix(n_components, 1, h_explained_var_ratio.begin());
  result["singular_values"] =
    Rcpp::NumericMatrix(n_components, 1, h_singular_vals.begin());
  result["mean"] = Rcpp::NumericMatrix(n_cols, 1, h_mu.begin());
  result["noise_variance"] = h_noise_vars.front();
